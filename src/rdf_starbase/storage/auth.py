...
class APIKey:
    ...
    key_id: str                          # Public identifier (prefix of key)
...
class RateLimitState:
    ...
    def check_query_limit(self, limit: int | None) -> bool:
        """Check if query is allowed under rate limit."""
        if limit is None:
            return True
        self.reset_if_needed()
        return self.query_count < limit
    
    def check_ingestion_limit(self, triple_count: int, limit: int | None) -> bool:
        """Check if ingestion is allowed under rate limit."""
        if limit is None:
            return True
        self.reset_if_needed()
        return self.triple_count + triple_count <= limit
...
class RateLimitExceeded(Exception):
    ...
    def __init__(self, key_id: str, limit_type: str, limit: int):
        self.key_id = key_id
